...
import { 
    ICompletionRequest, 
    ICompletionResponse } from "./interfaces/completion.js";
import { 
    IAllModelResponse, 
...
class GigaChat {
    ...
    public async createToken(): Promise<ITokenResponse> {
        try {
            ...
            const data: {scope: string} = {scope: ''}
            
            if(this.isPersonal) {
                data["scope"] = this.scopeForPersonal
            }
            else {
                data["scope"] = this.scopeForCorporation
            }
    ...
    public async completion(data: ICompletionRequest): Promise<ICompletionResponse> {
        ...
        try {
            const response = await this.post(path, 
                Object.assign({stream: false, update_interval: 0}, data)
            );
